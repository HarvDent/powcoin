...
uint256 hashGenesisBlock("0x8e7991c9663acffe50158a650aed2f1a708aac7289e9bee1c6fd6626a19437d0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Powcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Powcoin Signed Message:\n";
...
{
    // Powcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Powcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Powcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Powcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8e7991c9663acffe50158a650aed2f1a708aac7289e9bee1c6fd6626a19437d0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1405923882;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085909801;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PowcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PowcoinMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running Powcoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PowcoinMiner terminated\n");
        throw;
    }
}
